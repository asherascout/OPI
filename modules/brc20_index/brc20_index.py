...
cur.execute(
    """SELECT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'brc20_block_hashes{}') AS table_existence;""".format(
        report_name
    )
)
...
if create_extra_tables:
    cur.execute(
        """SELECT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'brc20_extras_block_hashes{}') AS table_existence;""".format(
            report_name
        )
    )
    if cur.fetchone()[0] == False:
