...
cur.execute(
    """SELECT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'brc20_block_hashes') AS table_existence;"""
)
...
if create_extra_tables:
    cur.execute(
        """SELECT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'brc20_extras_block_hashes') AS table_existence;"""
    )
    if cur.fetchone()[0] == False:
...
if cur_metaprotocol.rowcount == 0:
    print(
        "ord_network_type not found, main db needs to be recreated from scratch or fixed with index.js, please run index.js or main_index"
    )
    sys.exit(1)
...
def is_positive_number(s, do_strip=False):
    try:
        ...
        return False  ## has to be a string
...
def get_number_extended_to_18_decimals(s, decimals, do_strip=False):
    ...
    if "." in s:
        ...
        if (
            len(s.split(".")[1]) > decimals or len(s.split(".")[1]) == 0
        ):  ## more decimal digit than allowed or no decimal digit after dot
            return None
...
def is_used_or_invalid(inscription_id):
    ...
    cur.execute(
        """select coalesce(sum(case when event_type = %s then 1 else 0 end), 0) as inscr_cnt,
                        ...
                        from brc20_events where inscription_id = %s;""",
        (
...
def fix_numstr_decimals(num_str, decimals):
    if len(num_str) <= 18:
        ...
        if decimals < 18:
            num_str = num_str[: -18 + decimals]
    if num_str[-1] == ".":
        num_str = num_str[:-1]  ## remove trailing dot
    return num_str
...
def get_transfer_inscribe_event(inscription_id):
    ...
    if inscription_id in transfer_inscribe_event_cache:
        ...
        return event
    cur.execute(
        """select event from brc20_events where event_type = %s and inscription_id = %s;""",
        (
            event_types["transfer-inscribe"],
            inscription_id,
        ),
...
def get_last_balance(pkscript, tick):
    ...
    cur.execute(
        """select overall_balance, available_balance from brc20_historic_balances where pkscript = %s and tick = %s order by block_height desc, id desc limit 1;""",
        (pkscript, tick),
    )
    row = cur.fetchone()
    balance_obj = None
    if row is None:
        balance_obj = {"overall_balance": 0, "available_balance": 0}
    else:
        balance_obj = {"overall_balance": row[0], "available_balance": row[1]}
...
def deploy_inscribe(
    ...
    cur.execute(
        """insert into brc20_events (event_type, block_height, inscription_id, event)
    values (%s, %s, %s, %s);""",
        (
    ...
    cur.execute(
        """insert into brc20_tickers (tick, max_supply, decimals, limit_per_mint, remaining_supply, block_height)
    values (%s, %s, %s, %s, %s, %s);""",
        (tick, max_supply, decimals, limit_per_mint, max_supply, block_height),
    )
...
def mint_inscribe(
    block_height, inscription_id, minted_pkScript, minted_wallet, tick, amount
):
    global ticks, in_commit, block_events_str, event_types
    cur.execute("BEGIN;")
    in_commit = True
    ...
    values (%s, %s, %s, %s) returning id;""",
        (event_types["mint-inscribe"], block_height, inscription_id, json.dumps(event)),
    )
    event_id = cur.fetchone()[0]
    cur.execute(
        """update brc20_tickers set remaining_supply = remaining_supply - %s where tick = %s;""",
        (amount, tick),
    )
    ...
    cur.execute(
        """insert into brc20_historic_balances (pkscript, wallet, tick, overall_balance, available_balance, block_height, event_id) 
                values (%s, %s, %s, %s, %s, %s, %s);""",
        (
            minted_pkScript,
            minted_wallet,
            tick,
            last_balance["overall_balance"],
...
def transfer_inscribe(
    block_height, inscription_id, source_pkScript, source_wallet, tick, amount
):
...
def transfer_transfer_normal(
    ...
    )  ## negated to make a unique event_id

    cur.execute("COMMIT;")
    in_commit = False


def transfer_transfer_spend_to_fee(
    block_height, inscription_id, tick, amount, using_tx_id
):
    global in_commit, block_events_str, event_types
    cur.execute("BEGIN;")
    in_commit = True
...
def update_event_hashes(block_height):
    ...
    if len(block_events_str) > 0 and block_events_str[-1] == EVENT_SEPARATOR:
        block_events_str = block_events_str[:-1]  ## remove last separator
    block_event_hash = get_sha256_hash(block_events_str)
    cumulative_event_hash = None
    cur.execute(
        """select cumulative_event_hash from brc20_cumulative_event_hashes where block_height = %s;""",
        (block_height - 1,),
    )
    if cur.rowcount == 0:
    ...
    cur.execute(
        """INSERT INTO brc20_cumulative_event_hashes (block_height, block_event_hash, cumulative_event_hash) VALUES (%s, %s, %s);""",
...
def index_block(block_height, current_block_hash):
    ...
    if len(transfers) == 0:
        ...
        cur.execute(
            """INSERT INTO brc20_block_hashes (block_height, block_hash) VALUES (%s, %s);""",
    ...
    sttm = time.time()
    cur.execute(
        """select tick, remaining_supply, limit_per_mint, decimals from brc20_tickers;"""
    )
    ticks_ = cur.fetchall()
    ticks = {}
    for t in ticks_:
    ...
    for transfer in transfers:
        ...
        if sent_as_fee and old_satpoint == "":
            continue  ## inscribed as fee

        if content_type is None:
            continue  ## invalid inscription
        try:
            content_type = codecs.decode(content_type, "hex").decode("utf-8")
        except:
            pass
        content_type = content_type.split(";")[0]
        if content_type != "application/json" and content_type != "text/plain":
            continue  ## invalid inscription
        ...
        if js["op"] == "deploy" and old_satpoint == "":
            ...
            decimals = 18
            if "dec" in js:
                if not is_positive_number(js["dec"]):
            ...
            if decimals > 18:
                continue  ## invalid decimals
            max_supply = js["max"]
            if not is_positive_number_with_dot(max_supply):
                continue
            else:
            ...
            if "lim" in js:
                if not is_positive_number_with_dot(js["lim"]):
                    continue  ## invalid limit per mint
                else:
                    limit_per_mint = get_number_extended_to_18_decimals(
                        js["lim"], decimals
                    )
        ...
        if js["op"] == "mint" and old_satpoint == "":
            ...
            amount = js["amt"]
            if not is_positive_number_with_dot(amount):
        ...
        if js["op"] == "transfer":
            ...
            if old_satpoint == "":
                if not check_available_balance(new_pkScript, tick, amount):
                    continue  ## not enough available balance
                transfer_inscribe(
                    block_height, inscr_id, new_pkScript, new_addr, tick, amount
                )
            else:
                ...
                if sent_as_fee:
                    transfer_transfer_spend_to_fee(
                        block_height, inscr_id, tick, amount, tx_id
                    )
                else:
...
def check_for_reorg():
    ...
    if last_block_ord[1] == last_block[1]:
        return None  ## last block hashes are the same, no reorg
...
def reorg_fix(reorg_height):
    ...
    cur.execute("begin;")
    cur.execute(
        "delete from brc20_tickers where block_height > %s;", (reorg_height,)
    )  ## delete new tickers
    ## fetch mint events for reverting remaining_supply in other tickers
    cur.execute(
        """select event from brc20_events where event_type = %s and block_height > %s;""",
        (
    ...
    cur.execute(
        "delete from brc20_historic_balances where block_height > %s;", (reorg_height,)
    )  ## delete new balances
    cur.execute(
        "delete from brc20_events where block_height > %s;", (reorg_height,)
    )  ## delete new events
    cur.execute(
        "delete from brc20_cumulative_event_hashes where block_height > %s;",
        (reorg_height,),
    )  ## delete new bitmaps
    cur.execute(
        "SELECT setval('brc20_cumulative_event_hashes_id_seq', max(id)) from brc20_cumulative_event_hashes;"
    )  ## reset id sequence
    cur.execute(
        "SELECT setval('brc20_tickers_id_seq', max(id)) from brc20_tickers;"
    )  ## reset id sequence
    cur.execute(
        "SELECT setval('brc20_historic_balances_id_seq', max(id)) from brc20_historic_balances;"
    )  ## reset id sequence
    cur.execute(
        "SELECT setval('brc20_events_id_seq', max(id)) from brc20_events;"
    )  ## reset id sequence
    cur.execute(
        "delete from brc20_block_hashes where block_height > %s;", (reorg_height,)
    )  ## delete new block hashes
    cur.execute(
        "SELECT setval('brc20_block_hashes_id_seq', max(id)) from brc20_block_hashes;"
    )  ## reset id sequence
    cur.execute("commit;")
    reset_caches()
...
def check_if_there_is_residue_from_last_run():
    ...
    current_block = None
    if row[0] is None:
        current_block = first_inscription_height
    else:
    ...
    cur.execute(
        """select coalesce(max(block_height), -1) from brc20_historic_balances;"""
    )
    if cur.rowcount != 0 and cur.fetchone()[0] >= current_block:
        residue_found = True
    ...
    cur.execute(
        """select coalesce(max(block_height), -1) from brc20_cumulative_event_hashes;"""
    )
    if cur.rowcount != 0 and cur.fetchone()[0] >= current_block:
        residue_found = True
    ...
    if residue_found:
        print(
            "There is residue from last run, rolling back to " + str(current_block - 1)
        )
...
def check_if_there_is_residue_on_extra_tables_from_last_run():
    ...
    residue_found = False
    cur.execute(
        """select coalesce(max(block_height), -1) from brc20_unused_tx_inscrs;"""
    )
    if cur.rowcount != 0 and cur.fetchone()[0] >= current_block:
        residue_found = True
        print("residue on brc20_unused_tx_inscrs")
    cur.execute(
        """select coalesce(max(block_height), -1) from brc20_current_balances;"""
    )
    if cur.rowcount != 0 and cur.fetchone()[0] >= current_block:
        residue_found = True
    ...
    if residue_found:
        print(
            "There is residue on extra tables from last run, rolling back to "
            + str(current_block - 1)
        )
...
def reindex_cumulative_hashes():
    ...
    cur.execute(
        """select min(block_height), max(block_height) from brc20_block_hashes;"""
    )
    row = cur.fetchone()
    min_block = row[0]
    max_block = row[1]
    ...
    print(
        "Reindexing cumulative hashes from " + str(min_block) + " to " + str(max_block)
    )
    for block_height in range(min_block, max_block + 1):
        print("Reindexing block " + str(block_height))
        block_events_str = ""
        cur.execute(
            """select event, event_type, inscription_id from brc20_events where block_height = %s order by id asc;""",
            (block_height,),
        )
        rows = cur.fetchall()
        for row in rows:
            ...
            block_events_str += (
                get_event_str(event, event_type, inscription_id) + EVENT_SEPARATOR
            )
        update_event_hashes(block_height)
...
if cur.rowcount == 0:
    print(
        "Indexer version not found, db needs to be recreated from scratch, please run reset_init.py"
    )
    exit(1)
else:
    ...
    if db_version != DB_VERSION:
        ...
        if db_version not in CAN_BE_FIXED_DB_VERSIONS:
            print(
                "This version ("
                + db_version
                + ") cannot be fixed, please run reset_init.py"
            )
            exit(1)
        else:
            ...
            cur.execute(
                "alter table brc20_indexer_version add column if not exists db_version int4;"
            )  ## next versions will use DB_VERSION for DB check
            cur.execute(
                "update brc20_indexer_version set indexer_version = %s, db_version = %s;",
                (
                    INDEXER_VERSION,
                    DB_VERSION,
                ),
...
def reorg_on_extra_tables(reorg_height):
    cur.execute("begin;")
    cur.execute(
        "delete from brc20_current_balances where block_height > %s RETURNING pkscript, tick;",
        (reorg_height,),
    )  ## delete new balances
    rows = cur.fetchall()
        ...
        if cur.rowcount != 0:
            ...
            cur.execute(
                """insert into brc20_current_balances (pkscript, wallet, tick, overall_balance, available_balance, block_height)
                      values (%s, %s, %s, %s, %s, %s);""",
                (pkscript, balance[2], tick, balance[0], balance[1], balance[3]),
            )
    ...
    for row in rows:
        ...
        cur.execute(
            """INSERT INTO brc20_unused_tx_inscrs (inscription_id, tick, amount, current_holder_pkscript, current_holder_wallet, event_id, block_height)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)""",
            (
                inscription_id,
                new_event["tick"],
                int(new_event["amount"]),
                new_event["source_pkScript"],
                new_event["source_wallet"],
                event_id,
                block_height,
            ),
        )
    ...
    cur.execute(
        "delete from brc20_extras_block_hashes where block_height > %s;",
        (reorg_height,),
    )  ## delete new block hashes
    cur.execute(
        "SELECT setval('brc20_extras_block_hashes_id_seq', max(id)) from brc20_extras_block_hashes;"
    )  ## reset id sequence
    cur.execute("commit;")
...
def initial_index_of_extra_tables():
    ...
    cur.execute(
        """with tempp as (
                ...
                left join tempp2 t2 on t.inscription_id = t2.inscription_id
                where t2.inscription_id is null;""",
        (event_types["transfer-inscribe"], event_types["transfer-transfer"]),
    )
    rows = cur.fetchall()
    ...
    for row in rows:
        ...
        block_hash = row[1]
        cur.execute(
            """INSERT INTO brc20_extras_block_hashes (block_height, block_hash) VALUES (%s, %s);""",
...
def index_extra_tables(block_height, block_hash):
    ...
    cur.execute(
        "select max(block_height) as current_ebh_height from brc20_extras_block_hashes;"
    )
    if cur.rowcount > 0:
        res = cur.fetchone()[0]
        if res is not None:
    ...
    if len(balance_changes) == 0:
        ...
        for key in balance_changes_map:
            ...
            cur.execute(
                """INSERT INTO brc20_current_balances (pkscript, wallet, tick, overall_balance, available_balance, block_height) VALUES (%s, %s, %s, %s, %s, %s)
    ...
    if len(events) == 0:
        ...
        for row in events:
            ...
            if new_event["event_type"] == "transfer-inscribe":
                ...
                cur.execute(
                    """DELETE FROM brc20_unused_tx_inscrs WHERE inscription_id = %s;""",
                    (new_event["inscription_id"],),
                )
            else:
    ...
    """
        ...
        while ebh_tocheck_height <= main_block_height:
            ...
            if index_extra_tables(ebh_tocheck_height, block_hash):
                ...
                print(
                    "extra table data index failed for block: "
                    + str(ebh_tocheck_height)
                )
                return
    except:
        traceback.print_exc()
...
while True:
    ...
    cur_metaprotocol.execute(
        """SELECT coalesce(max(block_height), -1) as max_height from block_hashes;"""
    )
    ...
    cur_metaprotocol.execute(
        "select block_hash from block_hashes where block_height = %s;", (current_block,)
    )
    ...
    try:
        ...
        if create_extra_tables:
            ...
            check_extra_tables()
        if (
            max_block_of_metaprotocol_db - current_block < 10
            or current_block - last_report_height > 100
